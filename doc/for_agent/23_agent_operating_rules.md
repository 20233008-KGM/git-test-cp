...
- `vercel --prod`, `supabase functions deploy`, 기타 원격 배포 명령은 **사용자가 대화에서 명시적으로 지시한 경우에만** 실행한다.
- 구현/수정 작업만 요청받은 턴에서는 배포를 자동으로 실행하지 않는다.
- 배포가 필요해 보이면, 기본 동작은 "배포 준비 완료"까지만 수행하고 사용자의 최종 지시를 기다린다.
