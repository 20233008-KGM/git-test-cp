...
|----|--------|-----------|------|-----------|
| C-260525-7 | 2026-05-25 | starter·vision 점검 · #117~#121 인간 없이 구현 | ✅ | [`260525-5.md`](../for_agent/plans/260525-5.md) |
| C-260525-6 | 2026-05-25 | DB 업무 인간 불필요 — MCP 자동 적용 doc 정본 (`33`) | ✅ | `starter`·`23`·`28`·`09`·`25_human_collaboration` |
| C-260525-5 | 2026-05-25 | 밀린 Supabase SQL MCP 일괄 적용 · H-012 제거 · MCP 우선 정책 | ✅ | `28`·`25` · MCP 4건 마이그레이션 |
...
| # | 요청 요약 | 상태 | 처리 방법 |
|---|-----------|------|-----------|
| 83 | 강의자료 업로드 버킷 not found | ✅ | MCP `course_materials_*` · Storage RLS ([`260523-4`](../for_agent/plans/260523-4.md) T-204) |
...
| 87 | 교수 공지 등록 | ✅ | `assertProfessorCanManageAnnouncements` · RLS INSERT 정책 MCP (`ai_announcements_insert_policy`) |
...
| 103 | 산출물 등록 실패 | ✅ | MCP `subtitle` 컬럼 · 레거시 URL·Storage 오류 안내 ([`260525-3`](260525-3.md)) |
...
## 상세 (vision.md 추가요청 #117~#133)

| # | 요청 요약 | 상태 | 처리 방법 |
|---|-----------|------|-----------|
| 117 | 학생·교수 마이페이지 조회 | ✅ | `authLoading` 가드 · `showProfessorDashboard` · 교수 사이드 「마이페이지」([`260525-5`](../for_agent/plans/260525-5.md)) |
| 118 | DB 구조 점검·최적화 | ✅ | MCP `performance_fk_indexes` — 공지·Q&A·팀상세 FK 인덱스 10건 ([`260525-9`](../for_agent/plans/260525-9.md)) |
| 119 | 공지 「공지 작성」→ 작성 페이지 | ✅ | `/announcements/compose` · 목록 페이지 분리 |
| 120 | 공지 클릭 시 상세 | ✅ | `AppModal` · `announcement-detail-modal` |
| 121 | 교수 동료평가 조회 콘텐츠 작음 | ✅ | `CoursePeerReviewsOverviewPage` `w-full min-w-0` |
| 122 | 수업 등록 버튼·input 세로 중앙 정렬 | ✅ | `CoursesPage` join — label 아래 input+`M3Button` 동일 행 `h-[2.75rem]` ([`260525-6`](../for_agent/plans/260525-6.md)) |
| 123 | 산출물 모달 제목이 부제목 위 | ✅ | `TeamDeliverableSubmitModal` 필드 순서 ([`260525-6`](../for_agent/plans/260525-6.md)) |
| 124 | AI 진행요약 shimmer 로딩 중만 | ✅ | `TeamDetailPage` `insightLoading` · `GeminiShimmerPanel active={insightLoading}` |
| 125 | AI 진행요약 유의미한 인사이트 | ✅ | `buildTeamProgressInsight` 폴백 · Edge `progress-insight` · `fetchTeamProgressInsightFromEdge` · 강점/보완 목록 |
| 126 | 회원가입 비밀번호 8자 안내 | ✅ | `SignInPage` `signup-password-hint` · `minLength={8}` ([`260525-7`](../for_agent/plans/260525-7.md)) |
| 127 | 산출물 상단 최신 링크 1건 | ✅ | `TeamDetailPage` `team-deliverable-latest-link-banner` |
| 128 | AI 진행요약 중복 문구 | ✅ | summary·bullet 분리 · `normalizeProgressInsightForDisplay` |
| 129 | AI 진행요약 상용 수준 | ✅ | Edge mentor 프롬프트·subtitle/link · `recommend-troubleshooting` MCP/CLI 재배포 ([`260525-9`](../for_agent/plans/260525-9.md)) |
| 130 | 수업 등록 버튼 hover | ✅ | `cc-courses-join-submit` 밝기·그림자·`translateY` ([`260525-8`](../for_agent/plans/260525-8.md)) |
| 131 | 수업리스트 등록 버튼 hover | ✅ | `#130`과 동일 — 배너·빈목록 `cc-courses-join-submit` ([`260525-10`](../for_agent/plans/260525-10.md)) |
| 132 | AI 진행요약 Cursor급·소스 분석 | ✅ | Edge `source_code_samples` · `next_steps`·`architecture_risks`·`improvements` · UI ([`260525-10`](../for_agent/plans/260525-10.md)) |
| 133 | 산출물 게시판 상단 최신 링크 | ✅ | URL 보유 산출물 최신 1건 · 제목 직하 배너 ([`260525-10`](../for_agent/plans/260525-10.md)) |

---

## AI 유지 규칙 (에이전트)

1. `vision.md` 변경 시 `27`·본 문서 §상세(1~#133)·`26`을 같은 세션에 갱신한다.
